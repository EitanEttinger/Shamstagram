import { showSuccessMsg, showErrorMsg } from '../services/event-bus.service.js'
// import { storyService } from '../services/story.service.js'
import { storyService } from '../services/story.service.local.js'
import { addStory } from '../store/story.actions.js'
import { ImgUploader } from '../cmps/img-uploader'

import { useState } from 'react';
import { useSelector } from 'react-redux'

import EmojiPicker from 'emoji-picker-react';

export function StoryEdit() {
    const user = useSelector(storeState => storeState.userModule.user)
    const [createdStory, setCreatedStory] = useState(storyService.getEmptyStory(user))
    const [showPicker, setShowPicker] = useState(false);

    const onEmojiClick = (emojiObject, event) => {
        setCreatedStory(({ ...createdStory, txt: createdStory.txt + emojiObject.emoji }))
        // setShowPicker(false)
    }

    // const mutationObserver = new MutationObserver(listener)
    // mutationObserver.observe(document.body, { attributes: true })

    // function listener(mutationsList) {
    //     mutationsList.forEach(mutation => {
    //         if (mutation.attributeName === 'class') {
    //             setCreatedStory(storyService.getEmptyStory(user))
    //             // document.body.classList.remove('edit-post-open')
    //         }
    //     })
    // }

    async function onAddStory() {
        if (!createdStory.imgUrl) return
        try {
            await addStory(createdStory)
            showSuccessMsg(`Story added (id: ${createdStory._id})`)
        } catch (err) {
            showErrorMsg('Cannot add story')
        }
        closeAddMenu()
    }

    function onUploaded(imgUrl) {
        // onAddStory(imgUrl)
        setCreatedStory(prevState => ({ ...prevState, imgUrl: [imgUrl], time: Date.now() }))
        // if (document.body.classList.includes('edit-post-open')) console.log('object');
        // document.body.classList.add('edit-post-open')   
        // toggleAddMenu()
    }

    function handleChange({ target }) {
        let { value, type, name: field } = target
        // value = type === 'number' ? +value : value
        setCreatedStory(prevStory => ({ ...prevStory, [field]: value }))
    }

    function closeAddMenu() {
        document.body.classList.remove('edit-menu-open');
        setCreatedStory(storyService.getEmptyStory(user))
    }

    return (
        <div className='story-edit'>
            <div onClick={closeAddMenu} className='background-screen'></div>
            <div className='story-edit-container flex column'>
                {createdStory.imgUrl ? <section className='header flex'>
                    <span className='close-add-menu' onClick={closeAddMenu}>
                        <svg aria-label='Back' color='rgb(0, 0, 0)' fill='rgb(0, 0, 0)' height='24' role='img' viewBox='0 0 24 24' width='24'>
                            <title>Back</title>
                            <line fill='none' stroke='currentColor' strokeLinecap='round' strokeLinejoin='round' strokeWidth='2' x1='2.909' x2='22.001' y1='12.004' y2='12.004'></line>
                            <polyline fill='none' points='9.276 4.726 2.001 12.004 9.276 19.274' stroke='currentColor' strokeLinecap='round' strokeLinejoin='round' strokeWidth='2'></polyline>
                        </svg>
                    </span>
                    <h2>Create new post</h2>
                    <span onClick={onAddStory} className='share-btn'>Share</span>
                </section> :
                    <section className='header flex column'>
                        <h2>Create new post</h2>
                    </section>}
                <section className='create-post-container flex'>
                    {createdStory.imgUrl ?
                        <div className='post-edit flex row'>
                            <div className='img-container'>
                                <img src={createdStory.imgUrl} />
                            </div>
                            <div className='post-info'>
                                <div className='post-user-info'>
                                    <section className='user-info'>
                                        {/* <img src={logedinUser.imgUrl} /> */}
                                        <span>{createdStory.by.username}</span>
                                    </section>
                                    <form>
                                        <textarea type='text'
                                            name='txt'
                                            id='txt'
                                            placeholder='Write a caption...'
                                            value={createdStory.txt}
                                            onChange={handleChange}
                                        />
                                    </form>
                                </div>
                                <div className='post-info-footer'>
                                    <svg onClick={() => setShowPicker(val => !val)} aria-label='Emoji' color='rgb(0, 0, 0)' fill='rgb(0, 0, 0)' height='24' role='img' viewBox='0 0 24 24' width='24'>
                                        <title>Emoji</title>
                                        <path d='M15.83 10.997a1.167 1.167 0 1 0 1.167 1.167 1.167 1.167 0 0 0-1.167-1.167Zm-6.5 1.167a1.167 1.167 0 1 0-1.166 1.167 1.167 1.167 0 0 0 1.166-1.167Zm5.163 3.24a3.406 3.406 0 0 1-4.982.007 1 1 0 1 0-1.557 1.256 5.397 5.397 0 0 0 8.09 0 1 1 0 0 0-1.55-1.263ZM12 .503a11.5 11.5 0 1 0 11.5 11.5A11.513 11.513 0 0 0 12 .503Zm0 21a9.5 9.5 0 1 1 9.5-9.5 9.51 9.51 0 0 1-9.5 9.5Z'></path>
                                    </svg>
                                    <span>{createdStory.txt.length}/2,200</span>
                                </div>
                                <div className="app">
                                    <div className="picker-container">
                                        {showPicker && <EmojiPicker emojiVersion="3.0" onEmojiClick={onEmojiClick} />}
                                    </div>
                                </div>
                            </div>
                        </div> :
                        <div className='main flex column'>
                            <svg aria-label='Icon to represent media such as images or videos' color='rgb(0, 0, 0)' fill='rgb(0, 0, 0)' height='77' role='img' viewBox='0 0 97.6 77.3' width='96'>
                                <title>Icon to represent media such as images or videos</title>
                                <path d='M16.3 24h.3c2.8-.2 4.9-2.6 4.8-5.4-.2-2.8-2.6-4.9-5.4-4.8s-4.9 2.6-4.8 5.4c.1 2.7 2.4 4.8 5.1 4.8zm-2.4-7.2c.5-.6 1.3-1 2.1-1h.2c1.7 0 3.1 1.4 3.1 3.1 0 1.7-1.4 3.1-3.1 3.1-1.7 0-3.1-1.4-3.1-3.1 0-.8.3-1.5.8-2.1z' fill='currentColor'></path>
                                <path d='M84.7 18.4 58 16.9l-.2-3c-.3-5.7-5.2-10.1-11-9.8L12.9 6c-5.7.3-10.1 5.3-9.8 11L5 51v.8c.7 5.2 5.1 9.1 10.3 9.1h.6l21.7-1.2v.6c-.3 5.7 4 10.7 9.8 11l34 2h.6c5.5 0 10.1-4.3 10.4-9.8l2-34c.4-5.8-4-10.7-9.7-11.1zM7.2 10.8C8.7 9.1 10.8 8.1 13 8l34-1.9c4.6-.3 8.6 3.3 8.9 7.9l.2 2.8-5.3-.3c-5.7-.3-10.7 4-11 9.8l-.6 9.5-9.5 10.7c-.2.3-.6.4-1 .5-.4 0-.7-.1-1-.4l-7.8-7c-1.4-1.3-3.5-1.1-4.8.3L7 49 5.2 17c-.2-2.3.6-4.5 2-6.2zm8.7 48c-4.3.2-8.1-2.8-8.8-7.1l9.4-10.5c.2-.3.6-.4 1-.5.4 0 .7.1 1 .4l7.8 7c.7.6 1.6.9 2.5.9.9 0 1.7-.5 2.3-1.1l7.8-8.8-1.1 18.6-21.9 1.1zm76.5-29.5-2 34c-.3 4.6-4.3 8.2-8.9 7.9l-34-2c-4.6-.3-8.2-4.3-7.9-8.9l2-34c.3-4.4 3.9-7.9 8.4-7.9h.5l34 2c4.7.3 8.2 4.3 7.9 8.9z' fill='currentColor'></path>
                                <path d='M78.2 41.6 61.3 30.5c-2.1-1.4-4.9-.8-6.2 1.3-.4.7-.7 1.4-.7 2.2l-1.2 20.1c-.1 2.5 1.7 4.6 4.2 4.8h.3c.7 0 1.4-.2 2-.5l18-9c2.2-1.1 3.1-3.8 2-6-.4-.7-.9-1.3-1.5-1.8zm-1.4 6-18 9c-.4.2-.8.3-1.3.3-.4 0-.9-.2-1.2-.4-.7-.5-1.2-1.3-1.1-2.2l1.2-20.1c.1-.9.6-1.7 1.4-2.1.8-.4 1.7-.3 2.5.1L77 43.3c1.2.8 1.5 2.3.7 3.4-.2.4-.5.7-.9.9z' fill='currentColor'></path>
                            </svg>
                            <span className='txt'>Upload photos and videos here</span>
                            {<ImgUploader onUploaded={onUploaded} />}
                        </div>}
                </section>
            </div>
        </div>
    )
}

export function toggleAddMenu() {
    document.body.classList.toggle('edit-menu-open');
}